...
udc_saf = udc_saf_sim(N1:N2);
current_battery = current_battery_sim(N1:N2);
voltage_battery = voltage_battery_sim(N1:N2);
...
video_duration = 8.0;
fps            = 30;
output_file    = 'video_plot_dc_grid_power_1.mp4';
...
lim = @(x) deal(min(x), max(x));
[yi_min, yi_max] = lim(current_battery);
[yv_min, yv_max] = lim(udc_saf);
...
setup_ax(ax_v);
hl_udc = plot(ax_v,NaN,NaN,'Color',col_u,'LineWidth',3);
hl_ubat = plot(ax_v,NaN,NaN,'Color',col_v,'LineWidth',3);
hc_v  = xline(ax_v,0,'r--','LineWidth',0.8,'Alpha',0.7);
legend(ax_v,{'u_{saf,dc}','u_{bat,dc}'},'TextColor','w', ...
       'Color','none','EdgeColor',[0.4 0.4 0.4],'Location','northeast','FontSize',font_size_legend);
title(ax_v,'SAF Udc and Battery Udc','Color','w','FontSize',font_size_title,'FontWeight','normal');
...
setup_ax(ax_i);
hl_ibat = plot(ax_i,NaN,NaN,'Color',col_u,'LineWidth',3);
hc_i  = xline(ax_i,0,'r--','LineWidth',0.8,'Alpha',0.7);
legend(ax_i,{'i_{bat,dc}'},'TextColor','w', ...
       'Color','none','EdgeColor',[0.4 0.4 0.4],'Location','northeast','FontSize',font_size_legend);
title(ax_i,'Battery Current','Color','w','FontSize',font_size_title,'FontWeight','normal');
...
for k = 1:n_frames
    ...
    set(hl_udc,'XData',time(1:idx),'YData',udc_saf(1:idx));
    set(hl_ubat,'XData',time(1:idx),'YData',voltage_battery(1:idx));
    set(hc_v, 'Value',time(idx)); set(ht_v,'String',t_str);

    set(hl_ibat,'XData',time(1:idx),'YData',current_battery(1:idx));
